...
import (
	...
	"github.com/zhengpanone/mxshop/userop-web/initialize"
	myValidator "github.com/zhengpanone/mxshop/userop-web/validator"
	"go.uber.org/zap"
	"io"
	"net/http"
	"os"
...
func runFunction(cmd *cobra.Command, args []string) {
	...
	global.Logger.Info("用户操作服务userop-web服务注册到注册中心")
	...
	go func() {
		if err := server.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
			global.Logger.Panic(fmt.Sprintf("用户操作服务userop-web 启动失败：%v", err))
		}
	}()
...
func shutdown(server *http.Server, serviceId string, registerClient consul.RegistryClient) {
	...
	if err := registerClient.DeRegister(serviceId); err != nil {
		zap.S().Info("用户操作服务userop-web 在注册中心注销失败：", err.Error())
	}
	global.Logger.Info("用户操作服务userop-web 在注册中心注销成功：")

	// 调用Http实例的Shutdown方法 关闭服务器
	if err := server.Shutdown(ctx); err != nil {
		global.Logger.Fatal("用户操作服务userop-web 关闭错误", zap.Error(err))
	}
	global.Logger.Info("用户操作服务userop-web 关闭")
}
