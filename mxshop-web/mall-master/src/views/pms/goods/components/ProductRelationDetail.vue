...
import { fetchSubjectList } from "@/api/sms/subject";
...
const selectSubject = computed({
  get: () => {
    ...
    if (
      props.value.subjectProductRelationList == null ||
      props.value.subjectProductRelationList.length <= 0
    ) {
      return subjects;
    }
...
const selectPrefrenceArea = computed({
  get: () => {
    ...
    if (
      props.value.prefrenceAreaProductRelationList == null ||
      props.value.prefrenceAreaProductRelationList.length <= 0
    ) {
      return prefrenceAreas;
    }
    for (
      let i = 0;
      i < props.value.prefrenceAreaProductRelationList.length;
      i++
    ) {
      prefrenceAreas.push(
        props.value.prefrenceAreaProductRelationList[i].prefrenceAreaId
      );
    }
    return prefrenceAreas;
  },
  set: (newValue) => {
    ...
    for (let i = 0; i < newValue.length; i++) {
      props.value.prefrenceAreaProductRelationList.push({
