  ...
  <el-card class="form-container" shadow="never">
    ...
    >
      ...
      <el-form-item label="上级分类：">
        <el-select
          v-model="productCateInfo.parent_first"
          placeholder="请选择第一级分类"
          @change="chooseCateFirst()"
        >
          <el-option
        ...
        <el-select
          v-if="selectProductCateList.length > 0"
          v-model="productCateInfo.parent_second"
          placeholder="请选择二级分类"
        >
          <el-option
...
import { postCategorys, getCategoryDetail } from "@/api/pms/goods";
import { putCategorys, getCategoryList } from "@/api/pms/category";
...
const getProductAttrCateList = async () => {
  ...
  list.forEach((productAttrCate: any) => {
    const children =
      productAttrCate.productAttributeList?.map((attr: any) => ({
        label: attr.name,
        value: attr.id,
      })) || [];
    filterAttrs.value.push({
...
const chooseCateFirst = () => {
  ...
  const selectedCate = productCate.value.find(
    (item) => item.id === parentFirst
  );
  if (selectedCate) {
    selectProductCateList.value = defaultCate.value.concat(
      selectedCate.sub_category || []
    );
  }
};
...
const removeFilterAttr = (productAttributeId: number) => {
  ...
  const index = filterProductAttrList.value.findIndex(
    (item) => item.value !== null && item.value[1] === productAttributeId
  );
  if (index !== -1) {
