          ...
          <div style="padding: 10px; border-left: 1px solid #dcdfe6">
            ...
            <div>
              <v-chart
                ...
                autoresize
              />
              <div v-if="dataEmpty" class="empty">暂无数据</div>
            </div>
          </div>
        </el-col>
      </el-row>
...
const chartData = reactive({
  columns: [] as string[],
  rows: [] as { date: string; orderCount: number; orderAmount: number }[],
});
...
// 监听 chartData 自动生成 option
watch(
  [() => chartData.rows, () => chartSettings],
  () => {
    ...
    chartOption.value = {
      ...
      legend: { show: false },
      xAxis: {
        type: chartSettings.xAxisType === "time" ? "time" : "category",
        data: chartData.rows.map((row) => row.date),
      },
      yAxis: [{ type: "value" }, { type: "value" }],
      series: [
        {
          name: chartSettings.labelMap.orderCount,
          type: "line",
          smooth: true,
          areaStyle: chartSettings.area ? {} : undefined,
          data: chartData.rows.map((row) => row.orderCount),
        },
        {
          name: chartSettings.labelMap.orderAmount,
          type: "line",
          smooth: true,
          areaStyle: chartSettings.area ? {} : undefined,
          data: chartData.rows.map((row) => row.orderAmount),
          yAxisIndex: 1,
        },
      ],
    };
